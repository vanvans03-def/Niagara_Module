...
////////////////////////////////////////////////////////////////
        ...
        pollingThread = new Thread(() -> {
            ...
            while (isPolling) {
                try {
                    ...
                    if (Math.random() < 0.1) System.out.println("📊 Poll [" + getName() + "]: " + value);
                    ...
                    if (consecutiveErrors == 1 || consecutiveErrors % MAX_ERRORS == 0) {
                        System.err.println("❌ Poll Error [" + getName() + "] (x" + consecutiveErrors + "): " + e.getMessage());
                    }
                    try {
    ...
    // ✅✅✅ แก้ไขส่วน Write BACnet ให้สมบูรณ์ ✅✅✅
    private void writeBACnet(double val) throws Exception {
        ...
        // ❌ Block การเขียน AI (Analog Input)
        if (objectType == 0) {
        ...
        AccessController.doPrivileged((PrivilegedAction<Void>) () -> {
            ...
            try {
                ...
                byte[] tx = BACnetUtil.buildWritePropertyReal(
                        ...
                        (float) val,
                        invokeId,
                        16 // Priority 16 (Manual Operator)
                );
        ...
        AccessController.doPrivileged((PrivilegedAction<Void>) () -> {
            ...
            try {
                ...
                if (type.contains("coil") || type.contains("binary")) {
                    ...
                    request = new byte[] {0x00, 0x02, 0x00, 0x00, 0x00, 0x06, 0x01, 0x05, (byte)(regAddr >> 8), (byte)(regAddr & 0xFF), (byte)(outputVal >> 8), (byte)(outputVal & 0xFF)};
                } else {
                    int intVal = (int) val;
                    request = new byte[] {0x00, 0x02, 0x00, 0x00, 0x00, 0x06, 0x01, 0x06, (byte)(regAddr >> 8), (byte)(regAddr & 0xFF), (byte)(intVal >> 8), (byte)(intVal & 0xFF)};
                }
                out.write(request);
                out.flush();
                byte[] response = new byte[256];
